(function () {
  ...
  function init() {
    ...
    var bubbles = Array.from(document.querySelectorAll('.reader-control-bubble'));
    var controlGroups = Array.from(document.querySelectorAll('.reader-control-group'));
    var themePalettes = {
    ...
    var searchQuery = '';
    var searchScope = sessionStorage.getItem('reader:search-scope:' + encodedPath) || 'chapter';
    var searchMatches = [];
    ...
    function applySidebarWidth(value) {
      var width = value <= SIDEBAR_COLLAPSED ? SIDEBAR_COLLAPSED : clampSidebarWidth(value);
    ...
    function initSidebarResize() {
      ...
      if (sidebarToggle) {
        sidebarToggle.addEventListener('click', function (event) {
          ...
          var current = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--sidebar-w'), 10) || 0;
          if (current === SIDEBAR_COLLAPSED) {
            var restored = parseInt(localStorage.getItem(SIDEBAR_LAST_KEY) || '', 10) || 280;
      ...
      sidebarResizer.addEventListener('pointerdown', function (event) {
        ...
        var startWidth = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--sidebar-w')) || 280;
        ...
        function onEnd(endEvent) {
          ...
          var finalWidth = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--sidebar-w'), 10) || 0;
    ...
    function applyThemeToFrame(activeBg, activeFg, activeLh, activeSize, activePadX, activeFont) {
    ...
    function handleReaderShortcut(event) {
      ...
      if (event.repeat && event.key !== 'j' && event.key !== 'k' && event.key !== 'J' && event.key !== 'K' && event.key !== 'ArrowDown' && event.key !== 'ArrowUp') return;
      if (event.key === 'g') {
        event.preventDefault();
        if (pendingG) {
          clearPendingG();
    ...
    function updateSearchUI() {
      ...
      var hasMatches = searchScope === 'book' ? bookSearchMatches.length > 0 : searchMatches.length > 0;
      var currentIndexValue = searchScope === 'book' ? activeBookSearchIndex : activeSearchIndex;
      var totalMatches = searchScope === 'book' ? bookSearchMatches.length : searchMatches.length;
      if (searchCount) {
        if (!hasQuery) {
          ...
          searchCount.textContent = String(currentIndexValue + 1) + '/' + String(totalMatches);
        }
      }
    ...
    function updateSearchScopeUI() {
      ...
      if (searchScope === 'book') {
        ...
        searchScopeToggle.setAttribute('aria-label', 'Switch to chapter search');
        searchInput.placeholder = 'Search book';
        searchInput.setAttribute('aria-label', 'Search book');
        searchScopeIcon.innerHTML = '<path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>';
        return;
      }
      ...
      searchScopeIcon.innerHTML = '<path d="M8 6h10"/><path d="M8 12h8"/><path d="M8 18h6"/><path d="M4 6h.01"/><path d="M4 12h.01"/><path d="M4 18h.01"/>';
    }
    ...
    function persistSearchState() {
      ...
      sessionStorage.setItem('reader:search-state:' + encodedPath, JSON.stringify({
        query: searchQuery,
        scope: searchScope,
    ...
    function clearSearchHighlights() {
      var doc = frame.contentDocument;
      if (!doc) return;
      Array.from(doc.querySelectorAll('.reader-search-mark')).forEach(unwrapSearchMark);
      searchMatches = [];
      activeSearchIndex = -1;
      updateSearchUI();
    }
    ...
    function getReaderSearchPalette() {
      var pal = themePalettes[localStorage.getItem('reader:theme') || readerTheme] || themePalettes['solarized-dark'];
      var pool = [];
    ...
    function collectSearchTextNodes(doc) {
      ...
      var walker = doc.createTreeWalker(doc.body || doc.documentElement, NodeFilter.SHOW_TEXT, {
        acceptNode: function (node) {
          ...
          if (node.parentElement.closest('script, style, noscript, .reader-search-mark')) return NodeFilter.FILTER_REJECT;
    ...
    function findSearchMatches(query) {
      ...
      collectSearchTextNodes(doc).forEach(function (node) {
        ...
        for (var i = ranges.length - 1; i >= 0; i--) {
          nodeMatches.push(highlightTextNode(node, ranges[i].start, ranges[i].end));
        }
        nodeMatches.reverse();
    ...
    function setActiveBookSearchMatch(index) {
      ...
      activeBookSearchIndex = ((index % bookSearchMatches.length) + bookSearchMatches.length) % bookSearchMatches.length;
    ...
    function runBookSearch(query, noNavigate) {
      ...
      return fetch('/reader-search/' + encodedPath + '?q=' + encodeURIComponent(searchQuery)).then(function (r) {
        if (!r.ok) {
    ...
    function openBubble(targetBubble) {
      var current = targetBubble.querySelector('.reader-current-swatch, .reader-current-line, .reader-current-size, #reader-font-current');
    ...
    function updateNavHover(clientX, clientY, width, height) {
      ...
      var showPrev = inBottomBand && clientX <= edgeWidth && Boolean(prevChapter);
      var showNext = inBottomBand && clientX >= width - edgeWidth && Boolean(nextChapter);
    ...
    function bindFrameNavHover() {
      ...
      doc.addEventListener('mousemove', function (event) {
        var win = doc.defaultView;
        var width = (win && win.innerWidth) || doc.documentElement.clientWidth || 0;
        var height = (win && win.innerHeight) || doc.documentElement.clientHeight || 0;
    ...
    function updateReaderProgress() {
      ...
      var bodyEl = doc.body;
      var scrollTop = (win && (win.scrollY || win.pageYOffset)) || (docEl && docEl.scrollTop) || (bodyEl && bodyEl.scrollTop) || 0;
      var clientHeight = (win && win.innerHeight) || (docEl && docEl.clientHeight) || (bodyEl && bodyEl.clientHeight) || 0;
      ...
      var chapterFraction = scrollable > 0 ? Math.min(Math.max(scrollTop / scrollable, 0), 1) : 1;
      var chapterProgress = chapterFraction * 100;
      var bookProgress = ((currentIndex + chapterFraction) / chapterCount) * 100;
    ...
    function bindFrameScroll() {
      ...
      if (bodyEl) bodyEl.addEventListener('scroll', updateReaderProgress, { passive: true });
    ...
    function bindSlider(slider, initialValue, updateValue, storageKey, getStoredValue) {
    ...
    function bindFontChoices() {
      var choices = Array.from(fontMenu.querySelectorAll('.reader-font-choice'));
      choices.forEach(function (choice) {
        var saved;
        var previewing = false;
        choice.addEventListener('mousedown', function (event) {
    ...
    bindChoices(bgChoices, 'bg', function (value) { bg = value; });
    bindChoices(fgChoices, 'fg', function (value) { fg = value; });
    bindSlider(lhSlider, lh, function (value) { lh = value; }, 'reader:lh', function () { return lh; });
    bindSlider(sizeSlider, size.replace('px', ''), function (value) { size = value + 'px'; }, 'reader:size', function () { return size; });
    bindSlider(padSlider, padX.replace('px', ''), function (value) { padX = value + 'px'; }, 'reader:px', function () { return padX; });

    fetch('/api/fonts').then(function (r) {
      return r.json();
    }).then(function (fonts) {
    ...
    localStorage.setItem('lastRead', JSON.stringify({ path: encodedPath, chapter: currentIndex, title: readerTitle }));
